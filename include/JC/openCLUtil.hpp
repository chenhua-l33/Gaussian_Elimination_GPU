...
/**
 * @file openCLUtil.hpp
 * @brief OpenCL utility functions for device management and kernel execution
 *
 * This file provides a comprehensive set of utilities for:
 * - OpenCL device discovery and selection
 * - Platform and device information querying
 * - Kernel compilation and execution
 * - Performance profiling
 * - Error handling and status reporting
 *
 * The utilities are designed to simplify common OpenCL operations and
 * provide detailed diagnostic information for debugging and optimization.
 */
...
namespace jc
{
	 ...
	 *
	 * Attempts to read file from current directory first, then tries parent directory.
	 * Handles both Unix and Windows path separators.
	 *
	 * @param file_name Name/path of file to read
	 * @return String containing file contents
	 * @throws runtime_error if file cannot be found/read
	 */
	string fileToString(const string &file_name)
	{
		...
		if (!file_stream)
		{
			// Try parent directory with Windows path separator
			string file_name_up("..\\");
	...
	/**
	 * @brief Compile OpenCL source code into a program
	 *
	 * Creates and builds an OpenCL program from source code string.
	 * Provides detailed error messages on compilation failure.
	 *
	 * @param source_code OpenCL source code string
	 * @param context OpenCL context
	 * @param device Target device
	 * @param options Optional compiler options
	 * @return Compiled OpenCL program
	 * @throws cl::Error on compilation failure with build log
	 */
	...
	/**
	 * @brief Load and compile OpenCL program from file
	 *
	 * Convenience function that combines fileToString() and stringToProgram().
	 *
	 * @param file_name OpenCL source file path
	 * @param context OpenCL context
	 * @param device Target device
	 * @param options Optional compiler options
	 * @return Compiled OpenCL program
	 * @throws runtime_error if file cannot be read
	 * @throws cl::Error on compilation failure
	 */
	...
	/**
	 * @brief Execute and profile an OpenCL kernel
	 *
	 * Enqueues and times kernel execution using OpenCL profiling events.
	 * Waits for kernel completion and returns execution time.
	 *
	 * @param kernel Kernel to execute
	 * @param queue Command queue with profiling enabled
	 * @param global Global work size
	 * @param local Local work size (defaults to NULL range)
	 * @return Kernel execution time in nanoseconds
	 */
	cl_ulong runAndTimeKernel(const cl::Kernel &kernel, const cl::CommandQueue &queue, const cl::NDRange global, const cl::NDRange &local = cl::NullRange)
	{
		...
		return t2 - t1; // Return execution time in nanoseconds
	}
	// ** ** ** ** ** ** ** ** ** ** **
	// ********** GPU Info ************
	// ** ** ** ** ** ** ** ** ** ** **
	...
	/**
	 * @brief Get device name from OpenCL device object
	...
	/**
	 * @brief Get device name using platform and device IDs
	 *
	 * @param platformID OpenCL platform index
	 * @param deviceID Device index within platform
	 * @return Device name string, empty if invalid IDs
	 */
	...
	/**
	 * @brief Get device clock frequency in MHz
	 * @param device OpenCL device
	...
	/**
	 * @brief Get device local memory size in bytes
	 * @param device OpenCL device
	...
	/**
	 * @brief Get number of compute units (cores)
	 *
	 * Returns number of compute units, which are called:
	 * - Streaming Multiprocessors (SMs) on NVIDIA
	 * - Compute Units (CUs) on AMD
	 *
	 * @param device OpenCL device
	...
	/**
	 * @brief Get device warp/wavefront size
	 *
	 * Returns the native SIMD width of the device:
	 * - 32 for NVIDIA GPUs (warps)
	 * - 64 for AMD GPUs (wavefronts)
	 * - 1 for other devices
	 *
	 * @param device OpenCL device
	 * @return Warp/wavefront size
	 */
	...
	/**
	 * @brief Get NVIDIA compute capability version
	 *
	 * Returns the compute capability version for NVIDIA GPUs.
	 * See: https://en.wikipedia.org/wiki/CUDA#GPUs_supported
	 *
	 * @param device OpenCL device
	 * @return Compute capability as float (e.g., 7.5), 0 if not NVIDIA
	 */
	...
	/**
	 * @brief Get NVIDIA architecture name from compute capability
	 *
	 * Maps compute capability version to architecture name:
	 * - < 2.0: Tesla
	 * - 2.x: Fermi
	 * - 3.x: Kepler
	 * - 5.x: Maxwell
	 ...
	 * - 8.x: Ampere
	 *
	 * @param cc Compute capability version
	...
	/**
	 * @brief Get NVIDIA architecture name for device
	 * @param device OpenCL device
	 * @return Architecture name string
	 */
		...
		try
		{
			...
			for (int i = 0; i < platforms.size(); ++i)
			{
				...
				for (int j = 0; j < devices.size(); ++j)
				{
					...
					cout << " - " << deviceName << " on " << platformName << ": " << nbrComputeUnits << " cores " << clockFrequency << "MHz " << " vector width=" << nativeVectorWidthFloat << endl;
					// cout << "        OpenCL C Version        :" << openCLCVersion << endl;
					// cout << "        Maximum workgroup size  :" << maxWorkGroupSize << endl;
					// cout << "        Native vector width     :" << nativeVectorWidthFloat << endl;
					// cout << "        Timer resolution        :" << timerResolution << endl;
